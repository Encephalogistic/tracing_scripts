...
for i = 1:numfiles
    ...
    wallInd(i).wi = ones(1,length(gps(i).g));
    temp = NaN(1,length(gps(i).g));
    anglec(i).ac = NaN(1,length(gps(i).g));
    ...
    for l = 1:length(v1)
        ...
        temp = min(cclosest(i).cc,distancefunctions(gps(i).g', a_points, b_points, cos(ccent(i).cc(2)/3376200.00386)));
        wallInd(i).wi = wallInd(i).wi+(l-wallInd(i).wi).*(abs(cclosest(i).cc-temp)>10E-5);
        cclosest(i).cc = temp;
        %cclosest(i).cc = min(cclosest(i).cc,distancefunctions(gps(i).g', a_points, b_points, cos(ccent(i).cc(2)/3376200.00386)));
    end
    
    inCont = nan(size(reshape(cclosest(i).cc,size(p))));
    tempX = reshape(gps(i).g(:,1),size(p));
    tempY = reshape(gps(i).g(:,2),size(p));
    out = false;
    ...
    for k = 1:length(p(:,1))
        for j = 1:length(p(1,:))
            if a(i).a(k,j)==0&&out
                inCont(k,j)=nan;
            elseif a(i).a(k,j)==0&&~out
                out = ~inpolygon(tempX(k,j),tempY(k,j),c(i).c.X,c(i).c.Y);
                if ~out
                    inCont(k,j) = 1;
                end
            elseif a(i).a(k,j)~=0
    ...
    cclosest(i).cc = cclosest(i).cc .* reshape(double(inCont),size(cclosest(i).cc));
    %cclosest(i).cc(cclosest(i).cc == 0) = NaN;
    
    for k = 1:length(anglec(i).ac)
        pa = [v1(1,wallInd(i).wi(k)) v1(2,wallInd(i).wi(k))];
        pb = [v2(1,wallInd(i).wi(k)) v2(2,wallInd(i).wi(k))];
        pv = gps(i).g(k,:);
        
        pdist = cclosest(i).cc(k);
        padist = real(sqrt(( cos(ccent(i).cc(2)/3376200.00386)*(pv(1,1)-pa(1,1)))^2+(pv(1,2)-pa(1,2))^2));
        pbdist = real(sqrt(( cos(ccent(i).cc(2)/3376200.00386)*(pv(1,1)-pb(1,1)))^2+(pv(1,2)-pb(1,2))^2));

        if abs(pdist-padist) < 10E-5
            angle = 0-(radtodeg(atan2( cos(ccent(i).cc(2)/3376200.00386)*(pa(1,1)-pv(1,1)),pa(1,2)-pv(1,2)))-90);
        elseif abs(pdist-pbdist) < 10E-5
            angle = 0-(radtodeg(atan2( cos(ccent(i).cc(2)/3376200.00386)*(pb(1,1)-pv(1,1)),pb(1,2)-pv(1,2)))-90);
        else 
            angle = -radtodeg(atan2( cos(ccent(i).cc(2)/3376200.00386)*(pb(1,1)-pa(1,1)),pb(1,2)-pa(1,2)));
            [cpc] = cross([pb 0]-[pa 0],[pv 0]-[pa 0]);
            if cpc(3) < 0
                angle = angle+180;
            end
        end
    ...
    anglec(i).ac = anglec(i).ac .*reshape(double(inCont),size(anglec(i).ac));
    ...
    anglecm(i).acm = reshape(anglec(i).ac,size(p));
    distcm(i).dcm = reshape(cclosest(i).cc,size(p));

    figure
    hold on;
    surf(reshape(anglec(i).ac,size(p)), 'EdgeColor','none');
